...
func (api *KonsumensController) GetToken() {
	...
	if login.Username == "admin" && login.Password == "mypassword" {
		makeLogin := types.MakeLogin{}
		doLogin := types.DoLogin{Calllogin: makeLogin}
		token, cookie := doLogin.Calllogin.CekLogin()
		go api.Ctx.SetCookie("token", cookie, time.Now().Add(time.Hour*24).Unix(), "/")
...
func (api *KonsumensController) GetAllKonsumensMongoUpdate() {
	...
	result := make(chan structs.InsertKonsumen)
	go func() {
		for csr.Next(ctx) {
			var row structs.InsertKonsumen
			err := csr.Decode(&row)
	...
	sl := ChanToSlice(result).([]structs.InsertKonsumen)
