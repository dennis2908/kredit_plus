...
func (t *ElasticServer) SearchDataQuery(str structs.SearchDataStr, ttt *structs.SearchHits) error {
	query := fmt.Sprintf(`
	{
		"query": {
			"has_parent": {
	...
	json.Unmarshal(responseBody, &ttt)

	if len(ttt.Hits.Hits) == 0 {
...
func (t *ElasticServer) SearchAllData(k string, ttt *structs.SearchHits) error {

	req, err := http.Get("http://localhost:9200/multiple-env/_search")
	if err != nil {
		return err
	}
